...
# Сравнение времени выполнения рекурсивного и итеративного методов
while True:
    n = int(input("Введите натуральное число "))
    if n > 0:
        break
    else:
        print("Это число не является натуральным/превышает границу. Повторите ещё раз")
results = []
dp = [0] * (n + 1)
dp[0], dp[1] = 1, 1
...
for i in range(2, n + 1, 1):
    if i % 2 == 0:
        ...
        dp[i] = 5 * dp[i - 1] / math.factorial(2 * i) - dp[i - 3]
...
def recursive_F(n):
    if n < 2:
        return 1
    if n % 2 == 0:
        res = (-1) ** n * (2 * recursive_F(n - 1) + recursive_F(n - 3))
    else:
        res = 5 * recursive_F(n - 1) / math.factorial(2 * n) - recursive_F(n - 3)
    return res
...
results.append({'n': n, 'Recursive Result': int(result_recursive), 'Recursive Time (s)': recursive_time,
                'Iterative Result': int(result_iterative), 'Iterative Time (s)': iterative_time})
table = pd.DataFrame(results)
print("Таблица в виде отчёта:\n", table)
